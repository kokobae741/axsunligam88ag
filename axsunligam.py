...
def log(value, status='[G1]SING SABAR', color='[CC]'):
...
def log_replace(value, status='SING SABAR', color='[R1]'):
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            self.log('SING SABAR {} port {}'.format(self.inject_host, self.inject_port), color='[R1]')
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('[CC]LAGI NYERUM !!! '.format(self.proxy_host, self.proxy_port))
            ...
            self.log('[WIS OKEH]'.format(self.proxy_host, self.proxy_port), color='[P1]')
        except OSError:
